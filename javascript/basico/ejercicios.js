...
for (let i = 0; i < Array_Numerico.length; i++) {
  // Cambio aquí para recorrer los valores del arreglo
  if (Array_Numerico[i] % 2 === 0) {
    // Asegúrate de comprobar el valor del arreglo
...
for (let i = 0; i < Array_Numerico20.length; i++) {
  // Cambio aquí también
  if (Array_Numerico20[i] % 3 === 0 && Array_Numerico20[i] % 5 !== 0) {
    // Comprobamos el valor del arreglo
...
Inventario.forEach((producto) => {
  // Para recorrer correctamente los objetos
  console.log(
    "Nombre:",
    producto.Nombre,
    "Precio:",
...
const Categoria_Electrónica = Inventario.filter(
